    ...
    void GenerateRandomPositions(int count)
    {
        ...
        for (int i = 0; i < count; ++i) //원하는 위치 개수만큼 반복
        {
            ...
            do
            {
                ...
                newPos = new Vector3(x, y, 0); //무작위 위치 생성
...
public class AutoAttack : MonoBehaviour
{
    ...
    private float criticalDamage = 5f; //치명타 데미지
    ...
    public void Start()
    {
        ...
        {
            GameObject obj = Instantiate(attackEffectPrefab); //이펙트 프리팹 생성
    ...
    private void Update()
    {
        if (isAutoAttacking && (enemy == null || !enemy.activeInHierarchy))
        {
            ...
            if (newEnemy != null)
            {
                enemy = newEnemy;
                targetImage = enemy.transform.Find("Collider");
                Logger.Log("새로운 적 발견 및 자동 공격 시작");
            }
        }
    }
    ...
    public IEnumerator AutoAttackRoutine()
    {
        while (isAutoAttacking) //공격 중일 때만 반복
        {
            if (enemy != null)
            {
                ...
                Logger.Log("공격 주기:" + attackInterval);
